...
const CalendarView = {
  ...
  _renderDay(day, tasks, members, today) {
      ...
      <div class="calendar-day ${day.isCurrentMonth ? '' : 'calendar-day-outside'} ${isToday ? 'calendar-day-today' : ''}">
        ...
        <div class="calendar-day-items">
          ${dayEvents.slice(0, maxVisible).map(e => `
            <div class="calendar-event calendar-event-gcal" onclick="event.stopPropagation(); EventModal.open('${e.id}')" title="${this._esc(e.title)}">
              ...
              <span class="calendar-event-title">${this._esc(e.title)}</span>
            </div>
          `).join('')}
          ${dayTasks.slice(0, Math.max(0, maxVisible - dayEvents.length)).map(t => {
            const member = members.find(m => m.id === t.assignee_id);
            return `
              <div class="calendar-event calendar-event-task" onclick="event.stopPropagation(); TaskModal.open('${t.id}')" title="${this._esc(t.title)}">
                <span class="priority-dot priority-dot-${t.priority}" style="width:6px;height:6px"></span>
                <span class="calendar-event-title">${this._esc(t.title)}</span>
