...
export const TeamList = {
        ...
        <div class="tl-modal-body">
          ...
          <div class="tl-photo-section">
            ...
            <div class="tl-photo-actions">
              ...
              ${hasPhoto ? `<button class="btn btn-sm" style="color: var(--error)" onclick="TeamList.removePhoto()">Fjern foto</button>` : ''}
            </div>
          </div>
  ...
  onPhotoSelected(input: HTMLInputElement): void {
    ...
    if (file.size > 5 * 1024 * 1024) {
      (window as any).App.toast('Billedet er for stort (max 5 MB)', 'error');
      return;
    }
  ...
  async removePhoto(): Promise<void> {
    ...
    try {
      ...
      (window as any).App.toast('Foto fjernet', 'success');
      // Refresh modal
      this.openModal(this._editingId);
    } catch {
      (window as any).App.toast('Kunne ikke fjerne foto', 'error');
    }
  },
  ...
  async save(): Promise<void> {
    ...
    try {
      ...
      (window as any).App.toast(this._editingId ? 'Medarbejder opdateret' : 'Medarbejder oprettet', 'success');
      this.closeModal();
      (window as any).App.render();
    } catch (err: any) {
      if (btn) {
