...
export const EventModal = {
  ...
  async open(eventId: string): Promise<void> {
    const event: GoogleCalendarEvent | null = await GoogleCal.getEvent(eventId);
    if (!event) {
      (window as any).App.toast('Kunne ikke hente event', 'error');
      return;
    }
    ...
    const attendeeStatusIcon: Record<string, string> = {
      ...
      <div class="modal-panel">
        ...
        <div class="modal-body">
          ...
          <div class="event-detail-row">
            ...
            <span class="text-sm">${escapeHtml(event.location)}</span>
          </div>` : ''}
            ...
            <div class="event-attendees">
              ${event.attendees.map(a => `
                <div class="event-attendee">
                  ...
                  ${a.email !== a.name ? `<span class="text-xs text-tertiary">${escapeHtml(a.email)}</span>` : ''}
                </div>
              `).join('')}
            </div>
          </div>` : ''}
          ...
          <div class="event-detail-row">
            ...
            <span class="text-xs text-tertiary">Organiseret af ${escapeHtml(event.organizer)}</span>
          </div>` : ''}
        </div>
...
(window as any).EventModal = EventModal;
